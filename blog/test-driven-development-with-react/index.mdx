...
description: Test Driven Development explained with React, Vitest and Testing Library examples.
authors: [yakov]
tags: [blog, react, testing, tdd, vitest, javascript, typescript, testing-library]
keywords: [tdd, react, testing, vitest, javascript, typescript, testing-library]
...
When implementation comes first, we might be biased when writing the tests, since we already know the behavior works as expected, and we might have even seen it in action.
...
For this demo I chose to go with [Vite](https://vitejs.dev/) since it's fast, modern, and has great testing support with Vitest integration,
which is the natural choice for modern React applications.
...
![vitest-initial-run](dummy-spec.png)
...
* Increment
    ...
    * Click on button should increment count value by one
* Decrement
    * Button with "Decrement" label should be displayed
    * Click on button should decrement count value by one
* Reset
